...
function DocumentConversion(options, comments) {
    ...
    var transformConvertRun=options&&options.transformConvertRun?options.transformConvertRun:null;
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            var styleTo= style.to;
            var attributes=converStyleForaAtributes(element);
            if(styleTo._elements&&styleTo._elements[0]){
                if(styleTo._elements[0].tagName=="h1"||styleTo._elements[0].tagName=="h2"||styleTo._elements[0].tagName=="h3"){
                    if(attributes&&attributes.style){
                        styleTo._elements[0].attributes["style"]=attributes.style;
                    }
                }
            }
            return styleTo;
    ...
    function convertRun(run, messages, options) {
        ...
        //对外扩展方法 是一个神奇的扩展方法 用于背景色处理
        if (transformConvertRun) {
            try {
                // 执行转换函数并获取结果
                const transRun = transformConvertRun(run);
                // 检查返回值是否有效
                if (transRun && typeof transRun === 'object') {
                    // 安全地遍历对象属性
                    for (var elementKey in  transRun) {
                        if (transRun[elementKey]) {
                            paths.push(htmlPaths.element(elementKey, transRun[elementKey], { fresh: false }));
                        }
                    }
                }
            } catch (error) {
    ...
    function converStyleForaAtributes(element, options) {
        ...
        if(element.alignment&&element.alignment==='distribute'){
            styleMap.push({ key: 'text-align', value: "justify" })
        }
        else if (element.alignment && element.alignment !== 'both') {
